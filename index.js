...
function findIndexByProperty(arr, name) {
  ...
  if (!Array.isArray(arr)) {
    ...
    const findex = arr.findIndex((obj) => obj.name === name);
    // Повертаємо знайдений індекс або -1, якщо об'єкт не знайдено.
    return findex !== undefined ? findex : -1;
  }
}
...
function findLastEvenNumber(arr) {
  ...
  const flast = arr.findLast((num) => num % 2 === 0);
  return flast !== undefined ? flast : "Парних чисел не знайдено";
...
function getSubArrayAndConvertToString(arr, startIdx) {
  ...
  const underarr = arr.slice(startIdx);
...
function getPositiveArrayValues(arr) {
  ...
  const positiveArrayValues = arrayValues.filter((elem) => elem > 0);
...
function flattenSquareAndFilter(arr, threshold) {
  ...
  const flatresult = arr.flat().map((x) => x ** 2);
  ...
  return flatresult.filter((elem) => elem > threshold);
}
...
function expandArrayByN(arr, n) {
  ...
  const result = arr.flatMap((num) => Array.from({ length: n }, () => num));
